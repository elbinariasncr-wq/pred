 ...
 * Kalau file belum ada → buat baru pakai DEFAULT_DB.
 * Kalau sudah ada → pakai data yang tersimpan.
 */
function loadDB() {
...
/**
 * Simpan state saat ini ke database.json
 */
function saveDB() {
  const data = {
...
// Load saat bot pertama kali start
const loaded = loadDB();
const db = {
...
/**
 * Format teks prediksi dari input owner
 * Contoh input:
 *   - KEVIN VS MIKZ : KEVIN
 *   - ANDRA VS KENZ : KENZ
 */
...
bot.onText(/\/start/, (msg) => {
  ...
  bot.sendMessage(
    ...
    `_Daftarkan dirimu sebagai premium untuk akses prediksi terbaik\\!_`,
    { parse_mode: 'Markdown' }
  );
});
...
bot.onText(/\/freepred/, (msg) => {
  ...
  bot.sendMessage(
    ...
    `━━━━━━━━━━━━━━━━━━━━━━\n` +
    `🕐 *Update:* ${time}\n\n` +
    `_Mau predik yang lebih akurat & fix win? Upgrade Ke 👑 PREMIUM\\!_`,
    { parse_mode: 'Markdown' }
  );
});
...
bot.onText(/\/premiumpred/, (msg) => {
  ...
  if (!isPremium(userId) && !isOwner(userId)) {
    return bot.sendMessage(
      ...
      `Hubungi owner @clopc untuk mendapatkan akses premium\\!`,
      { parse_mode: 'Markdown' }
    );
  }
...
bot.onText(/\/setfreepred([\s\S]*)/, (msg, match) => {
  ...
  db.freePred = input;
  saveDB(); // 💾 Simpan ke database.json
...
bot.onText(/\/listprem/, (msg) => {
  ...
  if (db.premiumUsers.size === 0) {
    return bot.sendMessage(
      chatId,
      `📋 Belum ada user premium saat ini\\.`,
      { parse_mode: 'Markdown' }
    );
  }
...
bot.onText(/\/help/, (msg) => {
  ...
  bot.sendMessage(
    chatId,
    `📖 *DAFTAR COMMAND BOT PRED*\n` +
    `━━━━━━━━━━━━━━━━━━━━━━\n\n` +
    `*SEMUA USER:*\n` +
    `/start — Mulai bot\n` +
    `/freepred — Prediksi Gratis Gerbaru\n` +
